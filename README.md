...
```text
PC: psutil -> psi-send -> UDP/45891 -> USB Ethernet -> Pi: psi-display -> HDMI
```
...
sudo mkdir -p /opt/psi-screen
sudo chown "$USER" /opt/psi-screen
git clone YOUR_REPOSITORY_URL /opt/psi-screen
cd /opt/psi-screen
python3 -m venv --system-site-packages .venv
.venv/bin/pip install -e .
sudo cp deploy/psi-screen.service /etc/systemd/system/
sudo cp config/psi-screen.env.example /etc/psi-screen.env
sudo systemctl daemon-reload
sudo systemctl enable --now psi-screen
```
...
5. Allow outbound UDP port 45891 in the PC firewall and run `psi-send --host 10.55.0.2`.

USB gadget details vary across Pi OS images, so verify interface names with `ip link`. Regular Wi-Fi is a useful first test: pass the Pi's Wi-Fi address to `psi-send`.
...
psi-send --host 10.55.0.2 --interval 1
```
...
For packet authentication, set `PSI_SECRET` on the Pi and pass the same value using `psi-send --secret VALUE`. UDP telemetry is intentionally one-way and lossy: the newest sample matters, and an old sample should never block animation.

## Project layout

```text
src/psi_screen/display.py   renderer and overlay
src/psi_screen/receiver.py  non-blocking UDP receiver
src/psi_screen/sender.py    cross-platform PC collector
src/psi_screen/protocol.py  versioned optional-HMAC protocol
